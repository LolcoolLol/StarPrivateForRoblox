...
runcode(function()
    local AnticheatDisablerV2 = {["Enabled"] = false}
    RootRemover = GuiLibrary["ObjectsThatCanBeSaved"]["UtilityWindow"]["Api"].CreateOptionsButton({
        ...
        ["Function"] = function(callback)
            if callback then
                ...
                createwarning("Vape", "Disabled Anticheat!", 10)
            end
        end
    })
end)
...
runcode(function()
    ...
    RedChams = GuiLibrary["ObjectsThatCanBeSaved"]["RenderWindow"]["Api"].CreateOptionsButton({
        ...
        ["Function"] = function(callback)
                ...
                createwarning("Star Private", "Disabling Next Game", 3)
            end
        end
    })
end)
...
runcode(function()
    local part
    local part2
    local WalkAir = {["Enabled"] = false}
    WalkAir = GuiLibrary["ObjectsThatCanBeSaved"]["BlatantWindow"]["Api"].CreateOptionsButton({
        ["Name"] = "WalkAir",
        ["HoverText"] = "walk in air",
        ["Function"] = function(callback)
            if callback then
...
ChatTag[Players.LocalPlayer.Name] =
	{
        TagText = "Mont Costume",
        TagColor = Color3.new(255, 0, 0), -- chat tag color
    }
    local oldchanneltab
    local oldchannelfunc
    local oldchanneltabs = {}
...
runcode(function()
        ...
        Multiaura = GuiLibrary["ObjectsThatCanBeSaved"]["BlatantWindow"]["Api"].CreateOptionsButton({
            ...
            ["HoverText"] = "Use Vape KillAura For It to Work"
        })
    end)